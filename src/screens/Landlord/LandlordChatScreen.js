...
const LandlordChatScreen = ({ navigation }) => {
    ...
    const unsub = onSnapshot(
      ...
      snap => {
        const items = snap.docs.map(d => {
          const data = d.data();
          const isLandlord = data.landlordId === uid;
    ...
    <View style={styles.container}>
      ...
      {filtered.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyDesc}>Vous verrez ici les échanges avec vos étudiants.</Text>
        </View>
      ) : (
        <FlatList
          data={filtered}
